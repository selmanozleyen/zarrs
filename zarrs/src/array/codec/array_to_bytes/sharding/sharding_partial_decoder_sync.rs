...
impl ShardingPartialDecoder {
    ...
    pub fn new(
        ...
        // exactly then, and only if the caller wants them kept.
        if decoder.nested && decoder.sharding_options.subchunk_decoder_cache() {
...
struct SubchunkTask {
    /// This chunk's shard index entry, and the key its decoder is cached under.
    entry: u64,
